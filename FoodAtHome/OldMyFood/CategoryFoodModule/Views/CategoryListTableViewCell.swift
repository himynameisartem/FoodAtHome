...
class CategoryListTableViewCell: UITableViewCell {
    ...
    private func experationIndication(from dateIndicator: UIView, sign: UILabel, indicator: CGFloat?) {
        ...
        if indicator >= 0.5 {
            dateIndicator.isHidden = true
            sign.isHidden = true
        } else if indicator < 0.5 && indicator >= 0.2 {
    ...
    func configure(_ food: FoodRealm) {
        ...
        experationIndication(from: expirationDateIndicator, sign: expirationSign, indicator: food.distanceBetweenProductionAndExpiration())
    }
}
